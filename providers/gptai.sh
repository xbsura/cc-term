...
# ============================================================
# Provider: gptai.work
# Matches: gptai.work, api.gptclubapi.xyz
# ============================================================
PROVIDER_NAME="gptai.work"
ANTHROPIC_BASE_URL="https://api.gptclubapi.xyz"
MANAGEMENT_URL="https://gptai.work"
...
_gptai_call() {
    ...
    curl -sS --connect-timeout 8 --max-time 15 \
        "$MANAGEMENT_URL/api/proxy/$endpoint" \
        -H 'Content-Type: application/json' \
        -H "Origin: $MANAGEMENT_URL" \
        -H "Referer: $MANAGEMENT_URL/dashboard" \
        --data-raw "$payload" 2>/dev/null
}

_get_api_id() {
    # Use app_id if provided, otherwise auto-discover from API key
    if [[ -n "$APP_ID" ]]; then
        echo "$APP_ID"
        return
    fi
    [[ -z "$TOKEN" ]] && return
    local result
    result=$(_gptai_call "get-key-id" "{\"apiKey\":\"$TOKEN\"}")
    [[ -z "$result" ]] && return
...
case "$ACTION" in
    ...
    quota)
        ...
        USER_STATS=$(_gptai_call "user-stats" "{\"apiId\":\"$API_ID\"}")
        MODEL_STATS=$(_gptai_call "user-model-stats" "{\"apiId\":\"$API_ID\",\"period\":\"daily\"}")
...
# user-stats: { success, data: { usage: { total: {...} }, limits: {...} } }
...
# weekly reset countdown (resets every Monday)
now = datetime.now()
days_until_monday = (7 - now.weekday()) % 7
if days_until_monday == 0:
    days_until_monday = 7
reset_dt = now.replace(hour=0, minute=0, second=0, microsecond=0)
from datetime import timedelta
reset_dt = reset_dt + timedelta(days=days_until_monday)
delta = reset_dt - now
d = delta.days
...
model_lines = []
if model_data:
    # compute total cost for summary
    day_total_cost = sum(
