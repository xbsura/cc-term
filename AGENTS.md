...
- `install.sh` installs dependencies and deploys files into `~/.cc-terminal`.
- `bin/` contains the main launchers and Python utilities, including `cc-terminal`, `cc-provider-manager.py`, `cc-state-manager.py`, `cc-remote-server.py`, and `cc-relay-server.py`.
...
- `./install.sh` — install the toolkit into `~/.cc-terminal` on macOS.
- `./bin/cc-terminal -h` — verify the main CLI entry point and available subcommands.
...
- `bash -n install.sh bin/cc-terminal providers/*.sh` — shell syntax validation.
...
- Preserve the repo’s macOS-first assumptions and isolated `~/.cc-terminal` paths.
...
Do not commit real provider tokens, local state, or copied files from `~/.cc-terminal`. Redact sample URLs and secrets in examples, and keep provider-specific behavior inside `providers/` rather than hard-coding credentials in shared scripts.
